...
const getReleaseBranches = (version) => [
    'latest',
    `v${(0, semver_1.major)(version)}`,
    `v${(0, semver_1.major)(version)}.${(0, semver_1.minor)(version)}`,
];
// Enumerate the release branches to push to
const getBranches = async () => {
    var _a;
    const prefix = (0, core_1.getInput)('release_branch_prefix');
    // Get latest version through input
    const version = (0, core_1.getInput)('release_version');
    const [prereleaseName] = (_a = (0, semver_1.prerelease)(version)) !== null && _a !== void 0 ? _a : [];
    // Use the supplied prerelease branch name, otherwise extract it from the version
    const prereleaseBranch = (0, core_1.getInput)('prerelease_branch') || prereleaseName;
...
const run = async () => {
    ...
    const gitignore = (0, core_1.getInput)('gitignore');
    (0, core_1.info)('Appending to .gitignore');
    await (0, promises_1.appendFile)('.gitignore', `\n${gitignore}`);
    // Get branches to release to if no fixed branch is set
    const fixedBranch = (0, core_1.getInput)('fixed_branch');
    const branches = fixedBranch ? [fixedBranch] : await (0, get_branches_1.default)();
...
if (require.main === require.cache[eval('__filename')]) {
    run().catch((error) => (0, core_1.setFailed)(`Action failed with error ${error.toString()}`));
}
exports["default"] = run;
